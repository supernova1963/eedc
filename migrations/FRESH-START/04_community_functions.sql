...
-- ============================================
-- Ermöglicht Anonymous & Authenticated Users Zugriff auf öffentliche Daten
-- ohne RLS-Probleme

-- ============================================
-- 1. get_public_anlagen()
-- ============================================
-- Liefert alle öffentlichen Anlagen mit Basis-Informationen
...
AS $$
  SELECT
    ...
    CASE
      WHEN f.standort_genau THEN a.standort_plz
      ELSE LEFT(a.standort_plz, 2) || 'XXX'
    END as standort_plz,
    a.standort_ort,
    CASE
      WHEN f.standort_genau THEN a.standort_latitude
      ELSE NULL
    END as standort_latitude,
    CASE
      WHEN f.standort_genau THEN a.standort_longitude
      ELSE NULL
    ...
    f.profil_oeffentlich,
    f.kennzahlen_oeffentlich,
    f.monatsdaten_oeffentlich,
    f.investitionen_oeffentlich
  FROM anlagen a
...
-- Grant Zugriff für alle
GRANT EXECUTE ON FUNCTION get_public_anlagen() TO anon, authenticated;

COMMENT ON FUNCTION get_public_anlagen() IS 'Liefert alle öffentlichen PV-Anlagen mit Basis-Informationen';

-- ============================================
-- 2. get_community_stats()
-- ============================================
-- Liefert aggregierte Community-Statistiken
...
COMMENT ON FUNCTION get_community_stats() IS 'Liefert aggregierte Statistiken über alle öffentlichen Anlagen';

-- ============================================
-- 3. get_public_anlage_details()
-- ============================================
-- Liefert detaillierte Informationen zu einer einzelnen öffentlichen Anlage
...
BEGIN
  ...
  SELECT
    ...
    -- Komponenten (wenn freigegeben)
    CASE WHEN f.investitionen_oeffentlich THEN (
    ...
    -- Monatsdaten Summary (wenn freigegeben)
    CASE WHEN f.monatsdaten_oeffentlich THEN (
...
COMMENT ON FUNCTION get_public_anlage_details(uuid) IS 'Liefert detaillierte Informationen zu einer öffentlichen Anlage';

-- ============================================
-- 4. get_public_monatsdaten()
-- ============================================
-- Liefert öffentliche Monatsdaten einer Anlage (wenn freigegeben)
...
AS $$
  SELECT
    ...
    -- Autarkiegrad: (Direktverbrauch + Batterieentladung) / Gesamtverbrauch * 100
    CASE
    ...
    -- Eigenverbrauchsquote: Direktverbrauch / PV-Erzeugung * 100
    CASE
  ...
  WHERE md.anlage_id = p_anlage_id
    AND f.monatsdaten_oeffentlich = true
    AND a.aktiv = true
...
-- ============================================
-- Sucht öffentliche Anlagen nach PLZ, Ort oder Leistung
...
COMMENT ON FUNCTION search_public_anlagen IS 'Sucht öffentliche Anlagen mit Filtern';
...
DO $$
BEGIN
  RAISE NOTICE '✅ Community Functions erfolgreich erstellt:';
