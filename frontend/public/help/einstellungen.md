...
- **Sonstiges:** Kategorie (Erzeuger / Verbraucher / **Abgabe an Dritte** / Speicher / **Verbrauchszähler**) + Beschreibung; die Monatsdaten-Felder passen sich der Kategorie an. *Abgabe an Dritte* ist für Mieterstrom, Allgemeinstrom oder ein Nachbarhaus mit **Zähler an der Übergabestelle**: die Menge ist kein Eigenverbrauch und keine Netz-Einspeisung, eedc zieht sie vom Eigenverbrauch ab; der Erlös bleibt dein €-Feld (siehe [Bedienung → Abgabe an Dritte](HANDBUCH_BEDIENUNG.md)).
...
- **Wärmepumpe: der Wärme-Vorschlag ist eine Schätzung — und er rechnet nur mit dem Strom derselben Funktion.** Ohne Wärmemengenzähler schlägt eedc die Heizwärme als *Strom × JAZ* vor (das Feld sagt „geschätzt"). Basis ist der gemessene **Heizstrom**: der Betriebsart-Zähler „Strom Heizbetrieb", sonst „Strom Heizen" der getrennten Messung, sonst der Gesamtstrom — der aber nur, wenn im Monat kein Kühl-, Lüft- oder Warmwasser-Strom daneben steht (sonst wäre er nicht der Heizstrom, und es gibt keinen Vorschlag). Einen **Warmwasser**-Vorschlag gibt es nur mit getrennter Strommessung; ohne sie steckt die Gesamtwärme im Heizwärme-Vorschlag. Übernimmst du den Vorschlag, merkt sich eedc, dass der Wert geschätzt ist: Ersparnis und CO₂ rechnen damit weiter, eine **Arbeitszahl** entsteht daraus nicht — sie gäbe nur deine eingetragene JAZ zurück.
- **E-Auto: Tachostand statt gefahrene Kilometer.** Wer die Monatskilometer nicht kennt, trägt beim E-Auto den **Tachostand** am Monatsende ein — so, wie er im Auto steht. eedc rechnet daraus die gefahrenen Kilometer (Stand dieses Monats minus Stand des Vormonats), zeigt die Rechnung direkt unter dem Feld und trägt sie auf Wunsch in „Gefahrene km" ein. Der erste Monat hat noch keinen Anfang: dort trägst du die Kilometer einmal selbst ein, ab dem zweiten rechnet eedc. Läuft der Stand einmal rückwärts (Auto gewechselt), sagt eedc das und schlägt nichts vor. ⚠ Der Tachostand ist nur für die **Handeingabe** gedacht — ein Kilometerstand-Sensor (Auto-Integration, OBD, MQTT) gehört weiterhin auf „Gefahrene km", dort bildet eedc die Differenz selbst.
...
- **Energieprofil-Daten löschen:** anlage-spezifisch (Monatsdaten bleiben erhalten). Der Scheduler holt die **Messwerte** aus der HA-Langzeitstatistik nach — aber nur so weit, wie sie zurückreicht. Die **aufgezeichneten PV-Prognosen** (Grundlage des Genauigkeits-Vergleichs) lassen sich nicht neu berechnen und sind endgültig weg.
