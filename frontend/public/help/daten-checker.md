...
| **Keine PV-Module als Komponente angelegt** | ❌ ERROR | Ohne PV-Modul-Komponenten fehlen Erzeugungsdaten in der Aufschlüsselung. (Ein Balkonkraftwerk zählt als Erzeuger: Mit einem Balkonkraftwerk ohne PV-Module gibt es keine Meldung — es trägt Nennleistung, Ausrichtung und Neigung selbst, PVGIS-Prognose und String-Vergleich rechnen es wie einen String.) | Einstellungen → Komponenten → PV-Module hinzufügen. |
...
| **\[Name\]: SCOP-Werte fehlen (Modus: EU-Label SCOP)** | ⚠️ WARNING | Effizienz-Modus *SCOP*, aber `scop_heizung` und/oder `scop_warmwasser` fehlen. Einsparung wird nicht berechnet. **Gefragt wird nur nach Achsen, die es am Gerät gibt:** An einer Brauchwasser-Wärmepumpe heißt die Meldung *„SCOP Warmwasser fehlt"* — nach dem SCOP Heizung wird dort nicht gefragt; an einer Split-Klimaanlage umgekehrt *„SCOP Heizung fehlt"*. | Werte vom EU-Label / Datenblatt der WP eintragen. |
| **\[Name\]: COP-Werte fehlen (Modus: Getrennte COPs)** | ⚠️ WARNING | Modus *Getrennte COPs*, aber `cop_heizung` und/oder `cop_warmwasser` fehlen — dieselbe Achsen-Regel wie bei SCOP darüber. | Beide Werte eintragen oder Modus auf JAZ wechseln; hat dein Gerät nur eine der beiden Achsen, genügt deren Wert. |
...
| **\[Name\]: Heizwärmebedarf nicht gesetzt** | ℹ️ INFO | `heizwaermebedarf_kwh` fehlt — ohne gepflegten Bedarf rechnet die Jahres-Einsparungsschätzung **gar nicht**, die ROI-Zeile trägt dann „Nicht bewertet“ samt Begründung. Erscheint nur an Geräten, an denen eedc eine **Heiz-Achse** erwartet. | Geschätzten Jahres-Heizwärmebedarf in kWh eintragen (z. B. aus Energieausweis). |
| **\[Name\]: Warmwasserbedarf nicht gesetzt** | ℹ️ INFO | Das Gegenstück an einem Gerät **ohne** Heiz-Achse, also an einer **Brauchwasser-Wärmepumpe**: Nach dem Heizwärmebedarf fragt eedc dort nicht, aber `warmwasserbedarf_kwh` versorgt dieselbe Jahres-Einsparungsschätzung. Es bleibt bei **einem** Hinweis je Gerät — er nennt die Achse, die das Gerät hat. | Geschätzten Jahres-Warmwasserbedarf in kWh eintragen (~500 kWh je Person und Jahr). |
...
| **\[Name\]: Gesamtzähler kleiner als die Summe der Achsen (Monate)** | ⚠️ WARNING | Bei getrennter Strommessung mit **zusätzlichem** Gesamtzähler: *Strom Heizen* und *Strom Warmwasser* sind Teile des Gesamtverbrauchs und können zusammen nicht mehr sein. Meist misst der Gesamtzähler nur einen Teil des Geräts (nur das Außengerät, nur einen Stromkreis) — oder eine der Achsen zählt fremden Verbrauch mit. **eedc rechnet in diesen Monaten mit der Summe der Achsen**, damit nichts verloren geht. ⚠ Nur diese Richtung ist ein Fehler; ein **höherer** Gesamtzähler ist der Normalfall (Standby, Steuerung, Umwälzpumpen). | Im Monatsabschluss prüfen, welcher Zähler welchen Wert liefert. |
| **\[Name\]: Der Gesamtzähler misst deutlich mehr als die Achsen — misst er nur die Wärmepumpe? (Monate)** | ℹ️ INFO | Mehr als ein Viertel des Verbrauchs liegt weder auf *Strom Heizen* noch auf *Strom Warmwasser*; eedc führt diese Menge als **„nicht aufgeteilt"**. **Das ist oft richtig** — Standby, Steuerung und Umwälzpumpen laufen auf keiner der beiden Achsen. Es kann aber auch heißen, dass am Gesamtzähler noch etwas anderes hängt. **Eine Frage, kein Fehler:** eedc weiß nicht, was am Zähler hängt, und behauptet es nicht. | *Einstellungen → Datenquellen*, den Gesamtzähler prüfen. Stimmt er, ist nichts zu tun — der Hinweis bleibt als Auskunft stehen. |
...
| **\[Name\]: Warmwasser-Wärme in N Monat(en) gespeichert — eine Split-Klimaanlage hat keinen Warmwasserkreis** | ℹ️ INFO | An einem Gerät der Art **Luft-Luft (Klimaanlage)** steht ein Warmwasser-Wert. Solche Geräte haben keinen Warmwasserkreis; eedc bietet das Feld dort seit 2026 nicht mehr an, ältere Einträge blieben aber stehen — und sind im Monatsabschluss deshalb **nicht mehr erreichbar**. **Der Wert zählt nicht als Wärme des Geräts** — sonst stünde dort eine Ersparnis gegenüber der Heizung, die das Gerät nie erzeugt hat. | Stammt der Wert aus dem **Kühlbetrieb**, trag ihn unter *Nutzenergie Kuehlbetrieb* ein — daraus rechnet eedc die **Arbeitszahl Kühlen**. Den gespeicherten Wert selbst entfernst du mit dem Knopf **„Wert entfernen"** direkt an der Meldung; er nimmt alle genannten Monate mit und fragt vorher nach. eedc löscht und verschiebt nichts von allein. |
| **\[Name\]: \[Feld\] in N Monat(en) gespeichert — dieses Gerät führt das Feld nicht mehr** | ℹ️ INFO | Dasselbe für **jedes** Feld, das an einer Einstellung des Geräts hängt: Netzladung an einem Speicher ohne „lädt aus dem Netz", V2H-Entladung an einem E-Auto ohne V2H, Speicherwerte an einem Balkonkraftwerk ohne Speicher, getrennte Ströme Heizen/Warmwasser nach Abschalten der getrennten Strommessung. Wird der Schalter zurückgestellt, verschwindet das Feld aus dem Monatsabschluss — ein vorher gespeicherter Wert bleibt in der Zeile und ist nicht mehr erreichbar. Eine **0** meldet der Checker nicht. | Stimmt die Einstellung: **„Wert entfernen"** an der Meldung. Soll der Wert weiter zählen: die Einstellung am Gerät zurückstellen — dann erscheint das Feld wieder im Monatsabschluss. Ein Feld, das das Gerät führt, lässt sich über den Knopf **nicht** entfernen (das Backend weist es ab) — es gehört im Monatsabschluss geleert. |

> **Gefragt wird nur nach Größen, die eedc an deinem Gerät erwartet — und das steht in der Feld-Registry, nicht in der Bauart.** Die Wärmepumpenart darf dort **vorschlagen**: Bei **Luft-Luft (Klimaanlage)** ist die Heizwärme optional statt Pflicht (ein Wärmemengenzähler ist dort praktisch nie vorhanden), einen Warmwasserkreis gibt es gar nicht — der Checker verlangt deshalb weder die monatliche Heizwärme noch die Tages-Zusatzzähler aus §4.6 und erwartet bei getrennter Strommessung nur „Strom Heizen". Bei einer **Brauchwasser-Wärmepumpe** ist umgekehrt die Heizseite zurückgestuft: gefragt wird nach Warmwasser, nicht nach Heizwärme oder Heizstrom. Der **Stromverbrauch** bleibt überall Pflicht, die Stromauswertung funktioniert vollständig; JAZ/COP zeigen „—" mit Grund statt einer Scheinzahl. Eine Wärmepumpe **ohne** eingetragene Art gilt als klassische Wärmepumpe — eine fehlende Angabe schaltet die Erwartung nicht ab. Wer an seiner Klimaanlage doch einen Wärmemengenzähler hat, ordnet ihn weiterhin zu; die Registry nimmt nichts weg, sie stuft nur zurück.
>
...
>
> * *Alter Energiepreis nicht gesetzt* und *Heizwärmebedarf nicht gesetzt* versorgen die Ersparnis-Rechnung gegenüber einer **ersetzten Heizung**. Sie bleiben aus, sobald bei *Vergleich mit alter Heizung (ROI)* die Option **„Nichts ersetzt (Neubau)"** gewählt ist — bei jeder Wärmepumpe, unabhängig von der Bauart. Heizt du mit deiner Klimaanlage tatsächlich, wähle den ersetzten Energieträger; dann erscheinen sie, und die Wirtschaftlichkeit wird wie bei jeder anderen Wärmepumpe gerechnet. ⚠ **Beim Heizwärmebedarf kommt eine zweite Frage dazu: Hat das Gerät überhaupt eine Heiz-Achse?** Eine **Brauchwasser-Wärmepumpe** hat keine — dort fragt eedc stattdessen nach dem **Warmwasserbedarf** (eigene Zeile in der Tabelle). Auch das entscheidet die Feld-Registry und nicht die Bauart, und deshalb behält die **Klimaanlage** den Hinweis auf den Heizwärmebedarf: Wer mit ihr heizt, braucht ihn für seine Ersparnis. ⭐ **Dieselbe Regel gilt seit dem 14.09.2026 für die SCOP- und COP-Werte** — und der Hinweis auf den **Heizwärmebedarf** bleibt zusätzlich aus, wenn statt der beiden Einzelwerte ein **Gesamt-Wärmebedarf** gepflegt ist; die Rechnung liest ihn zuerst.
...
> **Wer hier ausgenommen ist** — damit der Befund nicht etwas verlangt, das es bei dir nicht geben kann: **Split-Klimaanlagen** (Wärmepumpenart *Luft-Luft*) werden nicht nach Wärme gefragt — die Feld-Registry stuft die Heizwärme dort zurück und kennt keinen Warmwasserkreis; eine **Brauchwasser-Wärmepumpe** wird nicht nach Heizwärme gefragt. Das **E-Auto** wird übersprungen, wenn eine **Wallbox** existiert (dort wird die Heimladung geführt) oder wenn es als **Dienstwagen** markiert ist. Stillgelegte und inaktive Komponenten zählen ohnehin nicht mit.
...
| Meldung | Severity | Bedeutung | Behebung |
|---------|----------|-----------|----------|
| **„\[Gerät\]": Betriebsmodus nicht zugeordnet — Heiz- und Kühlstrom bleiben zusammen** | ℹ️ INFO | Für dieses Gerät gibt es **keinen** der beiden Wege zur Aufteilung (siehe Kasten darunter). Der Stromverbrauch zählt vollständig und richtig; es fehlt nur die Aufteilung. **Welche Geräte den Hinweis bekommen:** Split-Klimaanlagen (dort ist Heizen und Kühlen über einen Zähler der Regelfall — die Bauart schlägt den Hinweis vor) **und** jede Wärmepumpe, an der eine Kühl-Größe zugeordnet oder gepflegt ist (Kühl-Leistung, Kältemenge, Kühl-Betriebsart) — sie kühlt nachweislich, gleich welcher Bauart. Eine Luft-Wasser-Wärmepumpe ohne jede Kühl-Spur bekommt ihn nicht. | *Einstellungen → Datenquellen*, beim Gerät das Feld **Betriebsmodus** zuordnen. In Home Assistant ist das die **climate-Entität** deines Geräts (meist `climate.…`) — sie zeigt Heizen/Kühlen/Aus. |
...
> **Und eine dritte Spur, die nicht bei dir liegt: die Einstrahlung der letzten Tage ist vorläufig.** Den Strahlungswert holt eedc beim Wetterdienst. Für Tage, die weiter als fünf Tage zurückliegen, kommt er aus dem endgültigen Messarchiv; für die Tage davor gibt es das Archiv noch nicht, eedc nimmt dort den Modellwert. Der ist im Mittel gut, an **bewölkten** Tagen aber deutlich zu klein — und ein zu kleiner Vergleichsmaßstab treibt die Performance Ratio nach oben, ohne dass an deiner Anlage etwas falsch wäre. **Seit eedc 4.0.39 holt eedc den endgültigen Wert von selbst nach**, sobald ein Tag alt genug ist — und ältere Tage, die noch einen vorläufigen Wert tragen, zieht eedc einmalig in der Nacht nach dem Update nach (nur die Wetterzeile, die gemessene Energie bleibt). Ein Verdacht, der nur an solchen Tagen hing, verschwindet damit von allein. Wenn dein Verdacht also **ausschließlich** auf den letzten Tagen sitzt und dort auf trübe Tage fällt, lohnt sich das Abwarten, bevor du in deiner Einrichtung suchst.

#### Befunde

| Meldung | Severity | Bedeutung | Behebung |
|---------|----------|-----------|----------|
| **Verdacht auf PV-Doppelerfassung (PR > 1 oder spez. Ertrag zu hoch)** | ⚠️ WARNING | An mindestens drei Tagen liegen die Kennzahlen über dem physikalisch Möglichen. | Zuerst *Einstellungen → Datenquellen* öffnen und prüfen, ob dieselbe Erzeugung an zwei Stellen zugeordnet ist. **Regel:** Entweder je Erzeuger ein eigener Zähler **oder** ein Anlagen-Summenzähler — nicht beides. **Findet sich dort nichts:** unter *Einstellungen → Anlage* die kWp sowie **Ausrichtung und Neigung** der Module prüfen — beide gehen in den Vergleichsmaßstab ein. **Und wenn die auffälligen Tage alle in den letzten Tagen liegen:** dort ist die Einstrahlung noch vorläufig, eedc zieht sie selbst nach (siehe Kasten oben). |
