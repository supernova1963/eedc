...
| **Amortisation** | Die **Dauer** bis zur Rückzahlung: Kapitaleinsatz ÷ Jahres-Ersparnis, in [Auswertungen → ROI](HANDBUCH_BEDIENUNG.md#42-roi) mit den tatsächlich erfassten Erträgen gerechnet und mit dem voraussichtlichen **Break-Even-Jahr** daneben (Anker = frühestes Anschaffungsjahr). Sie ist keine Messung, sondern eine Rechnung mit einer **Annahme über die Zukunft** — eedc unterstellt, dass keine weitere Instandhaltung anfällt, und schreibt diese Annahme neben jede Dauer. Wer an einer Komponente *Kosten/Jahr* pflegt, sieht sie dort als „inkl. … €/Jahr Betriebskosten" mitgenannt. **PDF-Finanzbericht, Oberfläche und HA-Sensor `amortisation_jahre` nennen dieselbe Größe** — bis August 2026 rechnete das PDF stattdessen Gesamtkosten ÷ eines Prognosefeldes, das sich nirgends pflegen ließ, und zeigte deshalb dauerhaft „—". |
| **Kapitaleinsatz** | Der Betrag, der sich zurückverdienen muss — der **Nenner** jeder Amortisations- und ROI-Rechnung. Er besteht aus den relevanten Anschaffungskosten **plus** den einmaligen **Ausgaben** aus dem Monatsabschluss (Reparatur, Ersatzteil) **minus** den einmaligen **Erträgen** (Förderung, THG-Quote): Geld, das nie ausgegeben wurde, muss nicht zurückkommen. Positionen, die zur ganzen Anlage gebucht sind, wirken auf die Gesamtzahl, nicht auf eine einzelne Zeile. **Alle vier Sichten teilen ihn** — *Auswertungen → ROI*, *Aussichten*, PDF-Finanzbericht und HA-Sensoren. |
| **Amortisations-Fortschritt** | Die **Messung** neben der Dauer: „wie viel des Kapitaleinsatzes ist bereits zurückgeflossen?" in Prozent — ohne jede Annahme über die Zukunft, deshalb auch nicht zu optimistisch. Steht als Kachel und je Zeile in [Auswertungen → ROI](HANDBUCH_BEDIENUNG.md#42-roi). Ein „—" heißt „dieser Komponente nicht zurechenbar" (z. B. eine anlagenweit gebuchte Förderung), nicht „null Euro"; negative Werte werden nicht auf 0 geschönt. |
...
| **SFML** | Solar Forecast ML — KI-basierte Ertragsprognose eines externen Dienstes (HA-Integration von Tom-HA). In eedc als **operative** Prognosequelle wählbar, sobald Home Assistant verbunden ist (Add-on **oder** Token-Anbindung). Ist sie gewählt, erscheint SFML im Prognosen-Vergleich als **Wert** — aber bewusst ohne Abweichungs-Spalte und gar nicht im Genauigkeits-Tracking. Siehe [Prognosen §2.5](HANDBUCH_PROGNOSEN.md#25-sfml-solar-forecast-ml--sichtbar-aber-bewusst-nicht-bewertet). |
...
| **Fremdanteil auf den Zählern** | Anwender-Angabe am Wärmepumpen-Gerät für zwei Lagen, die eedc nicht messen kann: Der Heizstab hängt am **Stromzähler**, seine Wärme aber nicht am Wärmemengenzähler (Strom zu groß) — oder ein zweiter Erzeuger speist denselben Heizkreis, den der **Wärmemengenzähler** misst, während sein Strom getrennt gezählt wird (Wärme zu groß) — ein Gas-/Ölkessel, oder ein elektrischer Heizstab. **Welche Lage vorliegt, entscheiden die Zähler, nicht das Gerät.** **Ändert keine Menge:** eedc lässt nur die Arbeitszahl weg und nennt den Grund. |
...
| **Netto-Ertrag (PV)** | Was die PV-Anlage einbringt: Einspeiseerlös + Eigenverbrauchs-Ersparnis + **gepflegter Erlös eines Geräts mit eigenem Vergütungssatz** (Abgabe an Dritte, sonstiger Erzeuger) (+ Sonstige Erträge − Sonderkosten), bei Regelbesteuerung **abzüglich der USt auf den Eigenverbrauch**. **Ohne** Netzbezug-Kosten — die fielen auch ohne PV an — und **ohne** Wärmepumpen-/E-Mobilitäts-Ersparnis. Ein Erzeuger unter „Sonstiges" (BHKW, Windrad, Wasserkraft) **zählt hier mit** — er speist hinter denselben Hauszähler, und was er einspeist oder an Netzbezug ersetzt, steckt in Erlös und Ersparnis. Bewertet wird dabei das **Gerät** nicht: es hat keine eigene Ertrags-Zeile und keine Wirtschaftlichkeit, sein Ertrag wird am Gerät als „Ertrag/Jahr" gepflegt. Kennzahl im Block „Finanz-Übersicht" ([Auswertungen → Finanzen](HANDBUCH_BEDIENUNG.md#3-auswertungen)). Siehe [Berechnungen §3.1](BERECHNUNGEN.md). |
...
| **Einstellungs-PIN (Sperre)** | Optionaler Schutz gegen versehentliches Verstellen, einzuschalten unter [Einstellungen → Anlage](HANDBUCH_EINSTELLUNGEN.md#21b-einstellungen-mit-einer-pin-schützen). **Ansehen ist nie gesperrt**, das Ändern verlangt die PIN einmal je Browser-Sitzung; hell/dunkel bleibt frei. Ausdrücklich **kein Benutzerkonto und keine Rollen** — eedc weiß nicht, wer davorsitzt, es gibt genau einen Schlüssel. Standardmäßig aus. |
...
| **Verbrauchszähler** | Eine Kategorie unter *Sonstiges* (#377): ein Gas-, Wasser-, Heizöl-, Pellet- oder Flüssiggaszähler. eedc führt seinen **Zählerstand** mit und zeigt, wie weit er sich im gewählten Zeitraum bewegt hat — mehr nicht. **Erfasst, nicht bewertet:** In Energiebilanz, Autarkie, Wirtschaftlichkeit, CO₂-Bilanz und den Gemeinschaftsdaten kommt er bewusst nicht vor, weil Gas- und Wasserkosten Haushaltskosten sind und nicht zur Rechnung der PV-Anlage gehören. Die Einheit (m³, l, kg, t, kWh) gehört zum Gerät und ist reine Anzeige — eedc rechnet nichts um. Siehe [Handbuch Einstellungen §3.4a](HANDBUCH_EINSTELLUNGEN.md). |
