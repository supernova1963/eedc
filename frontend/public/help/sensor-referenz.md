...
| `eedc_speicher_voll_um` | Uhrzeit „Speicher voll" aus der SoC-Simulation ab aktuellem Speicherstand — **inkl. Ladeverluste**: gerechnet mit dem gepflegten Wirkungsgrad des Speichers. Vorher lief die Rechnung verlustfrei und meldete deshalb zu früh. Die **Verbrauchsannahme** der Simulation ist das gewichtete 8-Wochen-Profil (jüngere Tage zählen stärker, Halbwertszeit 14 Tage; Kaskade gleicher Wochentag → Werktag/Wochenende → alle Tage). Attribute `profil_typ` (`gewichtet_8_wochen`), `profil_wochen`, `profil_halbwertszeit_tage`, `profil_stufe` (welche Kaskadenstufe griff), `profil_tage` und `verbrauch_annahme_kwh` (der Tagesverbrauch, mit dem gerechnet wurde) nennen sie. ⚠ **Ein anderes Modell als `eedc_verbrauchsprognose_heute_kwh`** (7-Tage-Profil der Live-Kachel) — wer beide in einer Automation verrechnet, sieht das an `profil_typ`. Ohne brauchbare Historie rechnet die Simulation **ohne Verbrauch** (`profil_typ: kein_profil`, `verbrauch_annahme_kwh: 0`). |
...
>
**`wp_ersparnis_euro` und `wp_cop_durchschnitt` je Wärmepumpe (ab 05.09.2026 dieselbe Rechnung wie Hub und Cockpit):** Die Ersparnis kommt aus derselben Layer-Formel wie der Komponenten-Hub — je Monat mit dem Tarif seines Stichtags, mit dem Monats-Gaspreis, mit den fixen Zusatzkosten der Altheizung, und **ohne den Kühlstrom** (Kühlen ersetzt keine Heizung; das Attribut `berechnung` nennt den herausgehaltenen Betrag). Ist die Wärme geschätzt (Strom × JAZ) oder hängt ein zweiter Erzeuger am Wärmezähler, trägt der Sensor den Vorbehalt als Attribut **`vorbehalt`** — dieselben Worte wie unter der Ersparnis im Hub; in *Einstellungen → HA-Export* steht er unter der Berechnung. Die Arbeitszahl kennt dieselben Sperren wie der Hub (geschätzte Wärme, Heizstab-Fremdanteil, zweiter Erzeuger). Für den Strom je Betriebsart gibt es Sensoren für Heizen, Kühlen und Warmwasser; **Lüften und Entfeuchten** haben bewusst keinen — sie sind Mengen ohne Kennzahl (Cockpit und Hub zeigen sie), und eine Automation braucht sie nicht.

**Ein Sensor, der seinen Wert verliert, bleibt nicht stehen (ab 05.09.2026):** Kann eedc einen Wert in einem Lauf nicht mehr bilden — die Arbeitszahl nach einem eingetragenen Fremdanteil, die Ersparnis nach „nichts ersetzt", ein Betriebsart-Sensor nach dem Entfernen seiner Quelle, der Speicher-Wirkungsgrad über 100 % —, setzt eedc den Sensor in Home Assistant auf **unbekannt**, statt den letzten Wert liegen zu lassen. Eine **gesperrte Kennzahl** trägt dabei ihren Grund im Attribut `grund` (dieselben Worte wie im Hub). Die Langzeitstatistik bekommt an dieser Stelle eine Lücke — das ist die richtige Aussage, keine Störung. Bis dahin blieb der letzte publizierte Wert stehen und lief stündlich in die Statistik weiter. Was nie erfasst war, bekommt weiterhin keinen Sensor.
...
| `eedc_speicher_voll_um` | Uhrzeit, zu der der Speicher voraussichtlich voll ist (Simulation ab **aktuellem** Ladestand, **mit** dem gepflegten Wirkungsgrad — bei mehreren Speichern dem niedrigsten). Rechnet mit dem gewichteten **8-Wochen-Verbrauchsprofil** — nicht mit dem 7-Tage-Profil von `eedc_verbrauchsprognose_heute_kwh`; die Attribute `profil_typ` (`gewichtet_8_wochen`), `profil_wochen`, `profil_halbwertszeit_tage`, `profil_stufe`, `profil_tage` und `verbrauch_annahme_kwh` nennen die Annahme (Details in [§8a](#8a-eedc-pv-prognose-nach-ha-exportieren-mqtt--ha-sensoren)). Ohne brauchbare Historie: `profil_typ: kein_profil`, `verbrauch_annahme_kwh: 0`. |
