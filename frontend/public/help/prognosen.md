...
>
> ⚠ **Das gilt für die eedc-Quelle — sie ist der Standard.** Hast du **Solcast oder SFML** eingestellt, folgen die Sichten deiner Wahl (siehe [§2.3](#23-solcast--optional-dritte-meinung) und [§2.5](#25-sfml-solar-forecast-ml--sichtbar-aber-bewusst-nicht-bewertet)), die **HA-/MQTT-Sensoren dagegen tragen weiterhin immer die eedc-eigene Prognose**. Beides ist beabsichtigt: Solcast und SFML liegen über ihre eigene HA-Integration ohnehin nativ in Home Assistant — ein zweiter Export wäre eine zweite Driftstelle. Bildschirm und Sensor zeigen dann also **bewusst** verschiedene Zahlen; die [Sensor-Referenz](SENSOR-REFERENZ.md) sagt es an der Stelle noch einmal.
...
**Wie eedc die Sensoren findet — und was zu tun ist, wenn es sie nicht findet.** eedc fragt Home Assistant, **welche Entitäten zu der Integration gehören**, und erkennt ihre Rolle an der internen Kennung, die die Integration selbst vergibt. Beides ist unabhängig davon, wie deine Entitäten heißen, in welcher Sprache du Home Assistant eingerichtet hast und ob du sie später umbenannt hast. Antwortet deine Home-Assistant-Version darauf nicht, fällt eedc auf die frühere Erkennung über den Namen der Entity-ID zurück — dann, und nur dann, kann eine abweichend benannte Entität unentdeckt bleiben. **Was eedc gefunden hat, steht unter *Einstellungen → Stammdaten* direkt unter der Quellenwahl**, Rolle für Rolle; dort steht auch, auf welchem der beiden Wege gesucht wurde.

> **Echte Stundenauflösung:** Ist SFML als Quelle gewählt, nutzt eedc SFMLs **eigenes Stundenprofil** (bis zu 3 Tage, stündlich — aus dem evcc-Prognose-Sensor, bei dir meist `…_evcc_solar_prognose`). Die SFML-Kurve im Tagesverlauf zeigt also SFMLs eigene Form, nicht die über die OpenMeteo-Strahlungskurve „verschmierte" Tagessumme. Fehlt dieser Sensor, fällt eedc auf das Tagesprofil `prognose_heute` (24 h) und notfalls auf die alte GTI-Verteilung zurück. (Das ist reine Treue zur selbstgewählten Quelle, **kein** Genauigkeits-Vergleich.)
...
- **Kurzfristig (7–14 Tage):** tägliche Erzeugungsschätzung aus OpenMeteo, kalibriert mit dem eedc-Lernfaktor, mit Wettersymbolen und Datenquelle-Kürzel je Tag (MS/D2/EU/EC/BM). SFML erscheint hier **nicht** als zweite Linie: Die Kurzfrist-Aussicht blickt 7–14 Tage voraus, SFML liefert drei. Wo SFML wirkt, steht es dabei — im Live-Cockpit (Solar-Aussicht, dort auch „verbleibend") und unter *Auswertungen → Prognose* mit eigener Spalte.
